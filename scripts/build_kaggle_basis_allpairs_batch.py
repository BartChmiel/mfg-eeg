...
logger = logging.getLogger("HCR-PCA")
...
def _basis_mixed(u_1d: np.ndarray, m: int) -> np.ndarray:
    """Legendre basis on [0,1], drop degree-0."""
    F = legendre_orthonormal(u_1d, m)
    return F[:, 1:]


def _compute_coeffs_all_pairs_for_lag(
    Fy: np.ndarray, Fz: np.ndarray, subtract_marginals: bool
) -> np.ndarray:
    """Compute HCR coefficients M[i,j,a,b]."""
    n_eff = Fy.shape[0]
    M = np.einsum("tcm,tdn->cdmn", Fy, Fz) / float(n_eff)
    if subtract_marginals:
        my = Fy.mean(axis=0)
        mz = Fz.mean(axis=0)
        M -= my[:, None, :, None] * mz[None, :, None, :]
    return M


def _normalize_series(X, mode, fs, cfg):
...
def main():
    cfg = load_config()
    ap = argparse.ArgumentParser()
    ap.add_argument("--root", required=True, help="Data directory")
    ap.add_argument("--out", required=True, help="Output .npz path")
    ap.add_argument("--mode", choices=["corr", "gc"], default="gc")
    ap.add_argument("--m", type=int, default=4)
    ap.add_argument("--pca-r", type=int, default=3)
    ap.add_argument("--epoch-len-s", type=float, default=2.0)  # Added missing arg
    ap.add_argument(
        "--per-phase", action="store_true", help="Build separate PCA per phase"
    )
    args = ap.parse_args()

    root = Path(args.root)
    files = sorted(root.glob("**/subj*_series*_data.csv"))
    if not files:
    ...
    # Metadata init
    _, X0, _, ch_names, _ = load_kaggle_data_with_events(
        str(files[0]), str(files[0]).replace("_data.csv", "_events.csv")
    )
    C = X0.shape[1]

    phase_keys = [f"{p0}__{p1}" for p0, p1 in PHASES] if args.per_phase else ["global"]
    storage = {
        pk: {
            "sum_x": np.zeros((C, C, K)),
            "sum_xxT": np.zeros((C, C, K, K)),
            "n": np.zeros((C, C), dtype=np.int64),
        }
        for pk in phase_keys
    }

    # Main Processing Loop
    for data_path in tqdm(files, desc="Processing files"):
        ev_path = Path(str(data_path).replace("_data.csv", "_events.csv"))
        if not ev_path.exists():
            continue

        _, X, E, _, _ = load_kaggle_data_with_events(str(data_path), str(ev_path))
        Y_all, Z_all = _normalize_series(X, args.mode, fs, cfg)
        cycles = extract_grasp_cycles(E, fs=fs)

        for cyc in cycles:
            for p0, p1 in PHASES:
                pk = f"{p0}__{p1}" if args.per_phase else "global"
                # Fixed call with epoch_len_s
                bounds = phase_window_bounds(
                    X.shape[0],
                    int(cyc[p0]),
                    int(cyc[p1]),
                ...
                a, b = bounds
                Yr, Zr = Y_all[a:b, :], Z_all[a:b, :]

                # Precompute basis
                Fy_full = np.stack(
                    [_basis_mixed(Yr[:, c], m) for c in range(C)], axis=1
                )
                Fz_full = np.stack(
                    [_basis_mixed(Zr[:, c], m) for c in range(C)], axis=1
                )

                for ms in [0, 50, 100, 150, 200]:
                    Ls = int(round(ms * fs / 1000.0))
                    n_eff = (b - a) - Ls
                    if n_eff <= 5:
                        continue

                    M = _compute_coeffs_all_pairs_for_lag(
                        Fy_full[:n_eff], Fz_full[Ls:], True
                    )
                    Xpair = M.reshape(C, C, K)

                    storage[pk]["sum_x"] += Xpair
                    storage[pk]["sum_xxT"] += np.einsum("cdk,cdl->cdkl", Xpair, Xpair)
                    storage[pk]["n"] += 1

    # Finalize PCA
    for pk, data in storage.items():
        mean, U, eigvals = (
            np.zeros((C, C, K)),
            np.zeros((C, C, r, K)),
            np.zeros((C, C, r)),
        ...
        for i in range(C):
            for j in range(C):
                if data["n"][i, j] < 2:
                    continue
                mu = data["sum_x"][i, j] / data["n"][i, j]
                cov = (data["sum_xxT"][i, j] / data["n"][i, j]) - np.outer(mu, mu)
                cov = 0.5 * (cov + cov.T)
                w, V = np.linalg.eigh(cov)
                idx = np.argsort(w)[::-1]
                mean[i, j], U[i, j], eigvals[i, j] = (
                    mu,
                    _fix_pca_signs(V[:, idx[:r]].T),
                    w[idx[:r]],
                )

        out_path = (
            args.out.replace(".npz", f"_{pk}.npz") if args.per_phase else args.out
        )
        np.savez_compressed(
            out_path,
            ch_names=ch_names,
